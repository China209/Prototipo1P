...
namespace BodegasAgricolas.Mantenimientos.Empleado
{
    public partial class AEEmpleado : Form
    {
        ...
        void CargarDatos()
        {
            try
            {
                string cadena = "SELECT idEmpleado,idCargo, dpi, nit, nombre, apellido, correo, telefono, direccion FROM empleado WHERE estado=1";
        ...
        private void dgridVista_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                sCadena = dgridVista.Rows[e.RowIndex].Cells["idEmpleado"].Value.ToString();
                if (sCadena == "")
                {
                    iID = 0;
                }
                ...
                {
                    iID = int.Parse(dgridVista.Rows[e.RowIndex].Cells["idEmpleado"].Value.ToString());
                }
                if (iID != 0)
                {
                    if (dgridVista.CurrentRow != null)
                    {
                        string cadena = "UPDATE empleado SET dpi='" + dgridVista.Rows[e.RowIndex].Cells["dpi"].Value.ToString() + "', nit='" + dgridVista.Rows[e.RowIndex].Cells["nit"].Value.ToString() + "', nombre='" + dgridVista.Rows[e.RowIndex].Cells["nombre"].Value.ToString()
                            ...
                            + "', direccion='" + dgridVista.Rows[e.RowIndex].Cells["direccion"].Value.ToString() + "' WHERE idEmpleado='" + iID + "';";
                        datos = new OdbcDataAdapter(cadena, cn.conexion());
                        dt = new DataTable();
                        datos.Fill(dt);
                        dgridVista.DataSource = dt;
        ...
        private void cmsDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string cadena = "UPDATE empleado SET estado=0  WHERE idEmpleado='" + iIDEliminar + "';";
                datos = new OdbcDataAdapter(cadena, cn.conexion());
                dt = new DataTable();
                datos.Fill(dt);
                dgridVista.DataSource = dt;
        ...
        private void dgridVista_CellMouseUp(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            {
                iIDEliminar = int.Parse(dgridVista.Rows[e.RowIndex].Cells["idEmpleado"].Value.ToString());
