...
{
    public partial class AECategoria : Form
    {
        ...
        void CargarDatos()
        {
            try
            {
                string cadena = "SELECT idCategoria, nombre, descripcion FROM categoria_producto WHERE estado=1";
        ...
        private void dgridVista_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                sCadena = dgridVista.Rows[e.RowIndex].Cells["idCategoria"].Value.ToString();
                if (sCadena == "")
                {
                    iID = 0;
                }
                ...
                {
                    iID = int.Parse(dgridVista.Rows[e.RowIndex].Cells["idCategoria"].Value.ToString());
                }
                if (iID != 0)
                {
                    if (dgridVista.CurrentRow != null)
                    {
                        string cadena = "UPDATE categoria_producto SET nombre='" + dgridVista.Rows[e.RowIndex].Cells["nombre"].Value.ToString() + "', descripcion='" + dgridVista.Rows[e.RowIndex].Cells["descripcion"].Value.ToString() + "' WHERE idCategoria='" + iID + "';";
                        datos = new OdbcDataAdapter(cadena, cn.conexion());
                        dt = new DataTable();
                        datos.Fill(dt);
                        dgridVista.DataSource = dt;
        ...
        private void cmsDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string cadena = "UPDATE categoria_producto SET estado=0  WHERE idCategoria='" + iIDEliminar + "';";
                datos = new OdbcDataAdapter(cadena, cn.conexion());
                dt = new DataTable();
                datos.Fill(dt);
                dgridVista.DataSource = dt;
        ...
        private void dgridVista_CellMouseUp(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            {
                iIDEliminar = int.Parse(dgridVista.Rows[e.RowIndex].Cells["idCategoria"].Value.ToString());
